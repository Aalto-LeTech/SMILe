...
object AnimatedPicture:
   ...
   *
   * @param sourceResourcePath The path to the source GIF file.
 ...
 *
 * @param frames            A sequence of `PictureElement` instances representing the frames of the animation.
 * @param frameDelays       A sequence of integers representing the delay in milliseconds before showing the next frame.
 * @param currentFrameIndex The index of the currently displayed frame.
 * @param lastFrameTime     The system time in milliseconds when the last frame was set. Defaults to the current system time.
 */
class AnimatedPicture(
    val frames: Seq[PictureElement],
   ...
   *
   * @return A new `AnimatedPicture` instance, possibly with an updated frame index, if the delay for the current frame has passed.
   */
  ...
  private def internalCopy(
      newFrames: Seq[PictureElement] = frames,
  ...
  override def scaleBy(horizontalFactor: Double, verticalFactor: Double): AnimatedPicture =
